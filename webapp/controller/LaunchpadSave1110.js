sap.ui.define([
	...
	return BaseController.extend("authenticator.controller.Launchpad", {
		...
		onInitWebSocket: function () {
			...
			this.byId("DynamicApps").setContent(oPopupBox);			
			// Remover o popup após 5 segundos
			setTimeout(() => {
				oPopupBox.destroy();
			}, 5000);
		...
		onOpenNotificationPopover: function (oEvent) {
			if (!this._oPopover) {
				...
				// Criar e configurar a List
				var oList = new sap.m.List();
				this._oPopover.addContent(oList);

				// Adicionar NotificationListItems com dados de exemplo
				var aNotifications = [
					{
						title: "Notificação 1",
						description: "Descrição da Notificação 1",
						datetime: "09/10/2023 12:00",
						priority: "Medium"
					},
					{
						title: "Notificação 2",
				...
				aNotifications.forEach(function (oNotification) {
					oList.addItem(new sap.m.NotificationListItem({
						...
						datetime: oNotification.datetime,
						priority: oNotification.priority
					}));
				});

				// Associar o Popover à View para permitir o lifecycle management correto
				this.getView().addDependent(this._oPopover);
			}

			// Abre o popover ao lado do botão de notificação
